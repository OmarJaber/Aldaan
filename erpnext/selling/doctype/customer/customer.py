...
class Customer(TransactionBase):
    ...
    def add_customer_account(self):
        ...
        if not accounts:
            ...
            acc_name = frappe.get_list("Account", filters={"account_name":self.customer_name}, fields=["name"])
            self.account = str(acc_name[0]['name'])
            frappe.msgprint("Account for {0} was successfully made".format(self.customer_name))
