...
class Workstation(Document):
    ...
    def get_workstation_additions(self):
        arr=[]
        try:
            for workstation_addition in self.workstation_additions:
                arr.append(workstation_addition.workstation_additions)

            additions = frappe.get_list("Workstation Additions Cost", fields='workstation_addition')
            for addition in additions:
                if addition['workstation_addition'] not in arr:
                    self.append('workstation_additions', {"workstation_additions": addition['workstation_addition']})
        except:
            addition=frappe.db.sql("select name from `tabWorkstation Additions Cost`")
            for i in addition:
                self.append('workstation_additions', {"workstation_additions": i[0]})
