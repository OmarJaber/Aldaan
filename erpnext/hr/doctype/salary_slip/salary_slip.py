...
class SalarySlip(TransactionBase):
    ...
    def get_overtime(self):
        ...
        if hours>0 and self.employee_type:
            ...
            self.gross_pay = self.givenross_pay + hours*hour_rate
    ...
    def get_worked_hours(self):
        ...
        absent_hours=0
        absent_days = frappe.db.sql("select count(name) from `tabAttendance` where employee='{0}' and status='Absent' and attendance_date between '{1}' and '{2}' and docstatus=1 ".format(self.employee,self.start_date,self.end_date))[0][0]
        if absent_days>=5:
            absent_days = absent_days+calendar.FRIDAY
        absent_hours = absent_days*8

        salary_structure = get_assigned_salary_structure(self.employee, self.end_date or getdate(nowdate()))
        if salary_structure:
            salary = frappe.db.sql("select amount from `tabSalary Detail` where parenttype='Salary Structure' and salary_component='Basic' and parent='{0}'".format(salary_structure))
            if salary:
